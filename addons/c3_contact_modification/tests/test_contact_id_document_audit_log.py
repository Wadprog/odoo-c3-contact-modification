...
class TestContactIdDocumentAuditLog(TransactionCase):
    ...
    def test_uploading_id_document_creates_chatter_log(self):
        ...
        self.assertEqual(len(partner.message_ids), initial_message_count + 1)
        self._assert_latest_log_contains(partner, "uploaded an ID document")
        self._assert_latest_log_contains(partner, self.env.user.display_name)
    ...
    def test_replacing_id_document_creates_replacement_log(self):
        ...
        self.assertEqual(len(partner.message_ids), initial_message_count + 1)
        self._assert_latest_log_contains(partner, "replaced the ID document")
        self.assertNotIn("uploaded an ID document", self._latest_message_body(partner))
    ...
    def test_deleting_id_document_creates_deletion_log(self):
        ...
        self.assertEqual(len(partner.message_ids), initial_message_count + 1)
        self._assert_latest_log_contains(partner, "deleted the ID document")
    ...
    def test_unchanged_id_document_write_does_not_create_log(self):
        ...
        self.assertEqual(len(partner.message_ids), initial_message_count)
    ...
    def test_company_contact_id_document_change_does_not_create_log(self):
        ...
        self.assertFalse(company.message_ids)
    ...
    def test_normal_contact_user_can_create_id_document_log(self):
        ...
        self._assert_latest_log_contains(partner, contact_user.display_name)
        self._assert_latest_log_contains(partner, "uploaded an ID document")
    ...
    def test_french_id_document_log_messages(self):
        ...
        partner.write({"c3_id_document": VALID_PNG_DATA})
        self._assert_latest_log_contains(partner, "a téléversé une pièce d'identité.")

        partner.write({"c3_id_document": VALID_JPEG_DATA})
        self._assert_latest_log_contains(partner, "a remplacé la pièce d'identité.")

        partner.write({"c3_id_document": False})
        self._assert_latest_log_contains(partner, "a supprimé la pièce d'identité.")
